...
fn format_ip_addr(addr: &Addr) -> Result<String> {
    match addr {
        Addr::V4(addr) => Ok(format!("{}.{}.{}.{}", addr[0], addr[1], addr[2], addr[3])),
        Addr::V6(addr) => Ok(format!(
            "{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}:{}",
            addr[0],
            addr[1],
            addr[2],
...
async fn tcp_transfer(stream: &mut TcpStream, addr: &Addr, address: &String, port: u16) {
    ...
    let client = match addr {
        Addr::V4(_) => TcpStream::connect(address.clone()).await,
        Addr::V6(x) => {
        ...
        Addr::Domain(_) => TcpStream::connect(address.clone()).await,
    };
